...
#   1. explicit argument:  python patch_sources.py /path/to/repo
#   2. current working directory, if it looks like the repo
#   3. walk up from this file (handles scripts/patch_sources.py)
# ---------------------------------------------------------------------------
def _find_repo_root() -> Path:
    def looks_like_repo(p: Path) -> bool:
        return (p / "desktop").is_dir() and (p / "backend").is_dir()

    if len(sys.argv) > 1:
        cand = Path(sys.argv[1]).resolve()
        if looks_like_repo(cand):
            return cand
        print(f"Given path is not a repo root: {cand}", file=sys.stderr)
        sys.exit(2)
    ...
    print("Could not locate the repository root "
          "(needs desktop/ and backend/).", file=sys.stderr)
    print("Run from the repo root, or pass the path:", file=sys.stderr)
    print("  python scripts/patch_sources.py /path/to/MIMODH", file=sys.stderr)
    sys.exit(2)
